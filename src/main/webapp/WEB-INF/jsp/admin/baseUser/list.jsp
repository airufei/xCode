...
<body>
	<table width="99%">
		<tr>
			<td>
				<div style="height: auto; background: #fafafa; padding: 0px;"
					class="easyui-panel" title="用户信息" iconCls="icon-search"
											...
											<table style="width:98%">
												...
												<tr>
													...
													<td style="width:35%;text-align:left;"><input
														class="easyui-textbox" style="width:80%;height:32px"
														name="username" maxlength="30" /></td>
												...
												<tr>
													...
													<td style="width:35%;text-align:left;"><input
														class="easyui-textbox" style="width:80%;height:32px"
														name="phone" maxlength="11" /></td>
													<td style="text-align:right;">微信号：</td>
													<td style="width:35%;text-align:left;"><input
														class="easyui-textbox" style="width:80%;height:32px"
														name="wechart" maxlength="20" /></td>
													<td style="width:16%;text-align:center;"><input
														id="btnSubmit" class="btn btn-primary" type="button"
														onclick="serach()" value="查询" /></td>
												</tr>
		...
		<tr>
			<td>
				<table id="dg" name="dg" title="用户信息" iconCls="icon-list"
	...
	function serach() {
		...
		var url = '<%=request.getContextPath()%>/admin/baseUser/list';
		$('#dg').datagrid({
			url : url,
			type : "post",
			datatype : "json",
   ...
   function add() {
		var title='添加用户信息';
		var url = '<%=request.getContextPath()%>/admin/baseUser/edit';
		var height=500;
		var width=700;
	...
	function edit(target) {
		...
		var id=row.id;
		var title='编辑用户信息';
		var url = '<%=request.getContextPath()%>/admin/baseUser/edit?id='+id;
		var height=500;
		var width=700;
			ShowiframeWindow(url,title,width,height);
	}
	 ...
	 function deleted(target)
	 {
		  ...
		  var row = $('#dg').datagrid('getSelected');
		  var id=row.id;
	      var url='<%=request.getContextPath()%>/admin/baseUser/delete?id='+ id;
